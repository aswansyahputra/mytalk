...
Berikut ini adalah baris kode yang digunakan untuk mengambil data dari 100 unggahan terakhir akun instagram [Tirto](https://instagram.com/tirtoid). Data tersebut kita simpan dalam sub-direktori `data-raw`. Data tersebut sudah disediakan dalam studi kasus ini sehingga Anda tidak perlu menjalankan baris kode di bawah ini.
...
Beberapa paket yang akan digunakan dalam studi kasus ini diantaranya adalah `jsonlite`, `listviewer`, dan `tidyverse`. Silakan pasang (jika belum) dan aktifkan tiga paket tersebut terlebih dahulu.
...
Selanjutnya silakan melakukan inspeksi terhadap `tirto_raw` untuk meilhat strukturnya. Anda dapat menggunakan fungsi `str` untuk melakukan hal tersebut, namun untuk mempermudah silakan Anda pergunakan fungsi `jsonedit()` dari paket `listviewer`. Pergunakan argumen `mode = "view"`!
...
Setelah melakukan inspeksi struktur data `tirto_raw`, Anda mungkin menyadari bahwa sejatinya konten data tersimpan sebagai elemen pertama dalam `list`.Ekstraklah elemen pertama tersebut dengan menggunakan fungsi `pluck()`! Kemudian kita juga akan melakukan beberapa proses untuk akhirnya menyimpan `list` tersebut ke dalam sebuah obyek `tibble` bernama `tirto_tbl` sebagai berikut:
...
> Selamat Anda telah berhasil menyelesaikan studi kasus ini! Sebagai tahap terakhir Anda dapat menyimpan `tirto_comments` sebagai berkas CSV atau lainnya. Perjalanan dari `JSON` menjadi `CSV` telah Anda lakukan dengan sukses :) 
