...
```{r import-data}
___ <- fromJSON("data-raw/tirtoid/___")
```
...
tirto_tbl %>%
  mutate(
    ...
    is_video = map(post, "___"),
    caption = map(post, list("edge_media_to_caption.edges", "node", "text")),
    tags = map(post, "___"),
    video_view = map(post, "___"),
    media_like = map(post, "edge_media_preview_like.count"),
    comments_username = map(post, list("comments.data", "owner", "___")),
    comments_time = map(post, list("comments.data", "___")),
...
tirto_tbl %>%
  mutate(
    post_time = map_int(post, "___"),
    is_video = map_lgl(post, "___"),
    caption = map_chr(post, list("edge_media_to_caption.edges", "node", "text")),
    tags = map(post, "tags"),
    video_view = map_int(post, "___"),
    media_like = map_int(post, "edge_media_preview_like.count"),
    comments_username = map(post, list("comments.data", "___", "username")),
    comments_time = map(post, list("___", "created_at")),
    comments_text = map(post, list("___", "text"))
  )
```
